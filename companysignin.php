...
<input type="text" name="company_name" placeholder="Company name" required autofocus />
...
<?php
   if(isset($_POST['signin'])){
	...
	$rs=mysql_query("select company_name,password from company where company_name='$companyname' and password='$pass'");
	 $row=mysql_fetch_array($rs);
	 $adname=$row[0];
	 $adpass=$row[1];
	
   if(($companyname==$adname)&&($pass==$adpass)){  
		session_start();
		$_SESSION['companyname']=$companyname;
      echo " <script>window.location = 'companycommon.php';</script>";
  ...
  if($password==$repeat){
   ...
   $con=mysql_connect("localhost","root") or die("Connection error !!! ");
   $db=mysql_select_db("tender management",$con) or die("Cannot connect to Database ...");
   	$rs1=mysql_query("select * from company");
	if($rs1){
   ...
   $rs=mysql_query("insert into company values($id,'$company_name','$passwordmd5','$mail',$mobile)") or die(mysql_error());
   
   if($rs){      
      // Mail sent to company 
	$msg = " \n Hi,\n		Congrats !!!Welcome to Online Tender ".$company_name." !  is successfully Signup . Here you can find tenders easily. Keep Going ... \n		Thank you.\n\n\n With Regards,\n Online Tender Team	...";

  // use wordwrap() if lines are longer than 70 characters
    $msg = wordwrap($msg,150);

   // send email
  mail($mail,"Online Tender Company Signup Confirmation ...",$msg);
	   echo"<script>alert('Your Company ID : '+$id)</script>";
